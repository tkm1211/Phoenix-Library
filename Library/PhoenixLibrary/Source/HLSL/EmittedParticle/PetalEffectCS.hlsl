...
void main(uint3 DTid : SV_DispatchThreadID, uint Gid : SV_GroupIndex)
{
    ...
    if (DTid.x < aliveCount)
    {
      ...
      //      particle.force *= sin((particle.maxLife * 0.5f) - particle.life) < 0.0f ? -1.0f : 1.0f;
            ...
            particle.position.x += particle.velocity.x * (particle.force.x * 7.25f) * dt;
            particle.position.y += particle.velocity.y * (particle.force.y * 1.25f) * dt;
            particle.position.z += particle.velocity.z * (particle.force.z * 7.25f) * dt;
