...
float4 main(float4 position : SV_POSITION, float2 texcoord : TEXCOORD) : SV_TARGET
{
    ...
    for (int i = 1; i < 3; i++)
    {
        fragmentColor += hdrColorMap.Sample(linearBorderSamplerState, texcoord + float2(offset[i] / width, 0.0)) * weight[i];
        fragmentColor += hdrColorMap.Sample(linearBorderSamplerState, texcoord - float2(offset[i] / width, 0.0)) * weight[i];
    }
#else
	...
	for (int i = 0; i < 2; i++)
	{
		fragmentColor += hdrColorMap.Sample(linearBorderSamplerState, texcoord + float2(offset[i], 0.0) / width) * weight[i];
		fragmentColor += hdrColorMap.Sample(linearBorderSamplerState, texcoord - float2(offset[i], 0.0) / width) * weight[i];
	}
#endif
