...
namespace Meta
{
	...
	{
		playerSkillAnalyzer->Update(metaData.playerScore, elapsedTime);
		dynamicDifficultyAdjuster->Update(playerSkillAnalyzer->GetSkillLevel(), playerSkillAnalyzer->GetDownScore(), elapsedTime);
		battleEnemyController->Update(dynamicDifficultyAdjuster->GetBattleEnemyState(), metaData, elapsedTime);
	}
	...
	//			worldState->playerAttacking = (currentPlayerState == Player::AnimationState::Attack);
	//		}
	//	}
