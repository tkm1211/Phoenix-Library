...
bool ImGui::DragScalarN(const char* label, ImGuiID* id, ImGuiDataType data_type, void* p_data, int components, float v_speed, const void* p_min, const void* p_max, const char* format, float power)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return false;
    ...
    for (int i = 0; i < components; i++)
    {
        id[i] = window->GetIDNoKeepAlive(i);
        PushID(i);
        if (i > 0)
            SameLine(0, g.Style.ItemInnerSpacing.x);
        value_changed |= DragScalar("", data_type, p_data, v_speed, p_min, p_max, format, power);
...
bool ImGui::DragFloat3(const char* label, ImGuiID id[3], float v[3], float v_speed, float v_min, float v_max, const char* format, float power)
{
    return DragScalarN(label, id, ImGuiDataType_Float, v, 3, v_speed, &v_min, &v_max, format, power);
}
